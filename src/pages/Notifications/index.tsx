...
function Main() {
  ...
  useEffect(()=>{
    ...
    const officeExist = office && user?.oficina?.filter((e:string)=> e.toLowerCase() === office.toLowerCase())[0]
    if(!officeExist){
        ...
        columns: [
          ...
          },
          {
            ...
            formatter(cell) {
              ...
              return `<div class="h-4 flex items-center">
                <div class="font-normal whitespace-nowrap">${new Date(response?.fecha_notif as string).toLocaleDateString()}</div>
              </div>`;
            },
          },
          {
          ...
          },
          {
            ...
            formatter(cell) {
              ...
              return `<div class="h-4 flex items-center">
              <div class="font-normal whitespace-nowrap">${oficina ? capitalizeFirstLetter(oficina) : ''}</div>
            </div>`;
            },
          },
          {
      ...
      if(cuil && field === 'cuil'){
        ...
        if(cuilIsValid){
          try{
            ...
            table.setData(`${baseUrl}/webapishared/Notificacion_digital/listNotifxcuil?cuil=${cuil}`)
        ...
        try{
          table.setData(`${baseUrl}/webapishared/Notificacion_digital/ListNotifxEstado?cod_estado=${estado}`)
        }catch(err){
          console.log(err)
        }
      }
            ...
            >
              <div>
                <div className="flex">
                  ...
                  {cuilInput.reset && <a className="self-center underline text-secondary pl-2" onClick={onResetFilter} href="">Resetear</a> }
                </div>
                {cuilInput.error && filter.cuil && <span className="text-warning">Ingresa un CUIL válido</span>}
             </div>
